...
#include "DFITCSECApiStruct.h"
    ...
    public:
        /**
         * 创建行情API实例
         * @pszLogDir: log所在的路径。如果pszLogDir为NULL，则不生成log。
         * @return 创建出的UserApi
         */
		static DFITCSECMdApi * CreateDFITCMdApi(const char* pszLogDir = "");
		...
		virtual int SubscribeStockMarketDataByExchange(const char *pExchang, int nRequestID) = 0;
		...
		virtual int UnSubscribeStockMarketDataByExchange(const char *pExchang, int nRequestID) = 0;
		...
		virtual int SubscribeSOPMarketDataByExchange(const char * pExchange, int nRequestID) = 0;
		...
		virtual int UnSubscribeSOPMarketDataByExchange(const char *pExchange, int nRequestID) = 0;
		...
		/**
		* 设置日志等级: 数值越大表是只记录更高级别的数据 (初始化之前调用 init)
		* @level @level 0.info 1.warning 2.error 3.fatal
		*/
		virtual void SetLogLevel(int level) = 0;

		/**
		* 是否记录通信日志 (初始化之前调用 init)
		* @val true/false
		*/
		virtual void SetWriteComm(bool val) = 0;
    protected:
